...
def handle_update(id, title, app):
    if not title:
        messagebox.showerror(title="Edit Task", message="Can not update with empty task!", parent=app)
    else:
...
def show_edit_task_frame(task, app):
    ...
    label = tk.Label(master=frame, text="Edit task:")
    label.grid(row=0, column=0, columnspan=2)

    # 1. Add an entry widget and show the task title
    entry = tk.Entry(master=frame)
    entry.insert(0, task["title"])   
    entry.grid(row=1, column=0, columnspan=2, pady=5)

    # 2. Add a button with text "Update" for saving the changes
    update_btn = tk.Button(master=frame, text="Update", command=lambda: handle_update(task["_id"], entry.get(), app))
    update_btn.grid(row=2, column=0, padx=5, pady=5)

    # 3. Add a button with text "Back / Cancel" to remove the frame
